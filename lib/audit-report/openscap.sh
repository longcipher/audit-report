        ...
        case "$family" in
            ...
            arch)
                if [[ -f "$dir/ssg-${distro_id}-ds.xml" ]]; then
                    ...
                    break
                elif [[ -f "$dir/ssg-archlinux-ds.xml" ]]; then
                    ds_file="$dir/ssg-archlinux-ds.xml"
                    break
                fi
                ;;
...
# Args: $1 - output directory, $2 - datastream file path (optional, auto-detected),
#       $3 - profile ID (optional, auto-detected)
...
openscap_run() {
    ...
    if [[ -z "$profile" ]]; then
        profile="$(openscap_detect_profile "$datastream")"
    fi
